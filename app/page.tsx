...
// Lista de empresas ficticias para el carrusel
const empresas = [
  "Acme Learning",
  "NovaTech",
  "EduWave",
...
// Tipo para estadísticas de repositorios de GitHub
type GHStat = { name: string; stars: number };

// Tipo para estado de widgets (APIs externas)
type Widgets = {
  gh: GHStat[];
...
export default function Landing() {
  // Estado de estadísticas generales
  ...
  // Posición del mouse para efectos visuales
  const [mousePosition, setMousePosition] = useState({ x: 0, y: 0 });
  // Toast
  const { toast } = useToast();

  // Carga inicial: estadísticas y talleres destacados
  useEffect(() => {
    apiFetch("/stats")
      .then(setStats)
      .catch(() => {
  ...
  // Carga de widgets externos (GitHub repos)
  useEffect(() => {
    let cancelado = false;
    ...
    async function cargarWidgets() {
      try {
        // Sólo repos GitHub (parallel + tolerancia a fallos individuales)
        const reposResult = await Promise.allSettled([
          fetchJSON("https://api.github.com/repos/vercel/next.js"),
          fetchJSON("https://api.github.com/repos/pallets/flask"),
        ]);

        const gh: GHStat[] = [];
        const [nextRes, flaskRes] = reposResult;
        ...
        if (
          flaskRes?.status === "fulfilled" &&
          flaskRes.value?.stargazers_count != null
        ) {
          gh.push({
            name: "Flask",
            stars: Number(flaskRes.value.stargazers_count),
          });
        }
  ...
  // Cálculo de ocupación promedio (asumiendo capacidad 30)
  const ocupacion =
    stats && stats.talleres > 0
      ? Math.min(
          100,
          Math.round((stats.registros / (stats.talleres * 30)) * 100)
        )
      : 0;
          ...
          <div className="grid md:grid-cols-4 gap-6">
            ...
            <div>
              ...
              <ul className="space-y-2 text-white/85 text-sm">
                ...
                <li className="flex items-center gap-2">
                  <Terminal className="h-4 w-4 text-white" /> Flask:{" "}
                  <span className="text-white">
                    {widgets.gh
                      .find((x) => x.name === "Flask")
                      ?.stars?.toLocaleString() ?? "—"}{" "}
