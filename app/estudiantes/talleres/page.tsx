...
import { BotonInscripcion } from "@/components/boton-inscripcion"
import { FiltrosTalleres } from "@/components/filtros-talleres"
...
export default function PaginaTalleres() {
  ...
  async function cargar() {
    ...
    try {
      ...
      // Cargar registros del usuario si está logueado
      const token = obtenerTokenEstudiante()
      if (token) {
        try {
          const registros = await apiFetch("/registrations/me", { token })
          setMisRegistros(registros.map((r: Taller) => r._id))
        } catch {
          // Si falla, simplemente no mostramos los registros
          setMisRegistros([])
        }
      }
    } catch (e: any) {
  ...
  return (
    <div className="min-h-screen bg-[#0b0e13] text-white">
      ...
      <section className="max-w-6xl mx-auto px-4 py-8">
        ...
        <FiltrosTalleres onChange={(f) => setFiltros(f)} valores={filtros} />
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {talleres.map((t) => (
                <div
                  key={`${t._id}-card`}
                  data-slot="card"
                  className="flex flex-col gap-6 rounded-xl py-6 shadow-sm border border-white/10 bg-white/5 text-white"
                >
                  <div data-slot="card-content" className="px-6 pt-6">
                    ...
                    {/* Botón de inscripción dentro de cada tarjeta */}
